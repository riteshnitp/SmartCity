...
})(jQuery);/*! Socket.IO.js build:0.9.11 */
	...
	$.msgchat = function(el, options)
	{
		...
		this.setchatdtls = function(lid,dtls,opts) {
			if(typeof opts.clbk_fnc != 'undefined' && $.trim(opts.clbk_fnc) != '') {
				...
				setTimeout(opts.clbk_fnc+"('"+lid+"',"+dtls+",'"+optary+"');", 100);
				if(lid == 'xhr') {
					this.ajx_cc = 'y';
					// console.log(options);
...
function addToContact(jqel) {
	...
	if ($.trim(cnm) != '') {
		...
		if (!aj_ip) {
			aj_ip = true;
			c_xhr = $.ajax({ url: url, type: 'POST', data:{ 'c':cnm, 'name':name }, success:function(resp) {
...
function setui() {
	...
	$('#contacts').click(function() {
		...
		if (!aj_ip) {
			aj_ip = true;
			c_xhr = $.ajax({ url: url, type: 'POST', data:{ 'name':name }, success:function(resp) {
				...
				if(rsp != null) {
					for(var e in rsp['con']) {
						...
						if(typeof rsp['con'][e] == "string") {
							clist = clist + '<div id="msc'+e+'" class="member pointer" title="'+rsp['con'][e]+'" ><label class="cmsr pointer" style="">'+rsp['con'][e]+'</label> <span class="rjct" style="float:right;" title="Remove"> &nbsp; x </span> <span class="a2g" title="Add To Group" style="float:right; display:'+a2g+';"> &nbsp; + </span></div>';
						} else {
							for(var cn in rsp['con'][e]) {
								...
								//
								clist = clist + '<div id="msc'+e+'" class="member pointer '+classname+'" title="'+cn+'"><label class="cmsr pointer" style="" title="(Last Seen: '+lst+')">'+cn+'</label> <span class="rjct" style="float:right;" title="Remove"> &nbsp; x </span> <span class="a2g" title="Add To Group" style="float:right; display:'+a2g+';"> &nbsp; + </span></div>';
								break;
							}
						}
					}
	...
	$('#ssrch').keyup(function(e) {
		if(e.keyCode == 13) {
			...
			if (!aj_ip) {
				aj_ip = true;
				c_xhr = $.ajax({ url: url, type: 'POST', data:{ 'qk':$('#ssrch').val(), 'name':name }, success:function(resp) {
					var rsp = $.parseJSON(resp);
					var list = '';
					for(var e in rsp) {
	...
	$('#mgrp').click(function() {
		...
		if(!aj_ip) {
			aj_ip = true;
			c_xhr = $.ajax({ url: url, type: 'POST', data:{ 'grpcon':grpcon, 'grpnms':grpnms, 'name':name }, success: function(resp) {
				if($.trim(resp) == 'success') {
					$('#contacts').trigger('click');
					alert('Group Updated');
				} else {
		...
		if (!$('#grpcon').is('[readonly]')) {
			...
			if(grpnms != '') { 	// && grpnms.indexOf(',') == -1
				...
				if(!aj_ip) {
					...
					c_xhr = $.ajax({ url: url, type: 'POST', data:{ 'grpnm':grpnm, 'name':name }, success: function(resp) {
						$('#grpcon').val('');
...
function setChatBoxes(vl, gid, bk) {
	...
	$(tbselc).click(function() {
		...
		if(typeof ms.mcevs != 'undefined' && ms.mcevs != null && typeof ms.chattype != 'undefined' && ms.chattype != null && ms.chattype == 'ws') {
			if(typeof relvl != undefined && relvl != null && relvl != '') {
				ms.mcevs.send('scmsg:=:'+gprfx+vl+'-'+relvl);
			} else {
...
function setChatUi(vl, gid, bk) {
	...
	if(typeof ms.mcevs != 'undefined' && ms.mcevs != null && typeof ms.chattype != 'undefined' && ms.chattype != null && ms.chattype == 'ws' && (typeof bk == 'undefined' || bk == null)) {
		if(typeof gid != undefined && gid != null) {
			ms.mcevs.send('scmsg:=:'+gprfx+vl+'-'+gid);
		} else {
			ms.mcevs.send('scmsg:=:'+vl);
		}
	}
...
function biCChat() {
	$('#clist > .member > .cmsr').click(function() {
		...
		if (!aj_ip) {
			aj_ip = true;
			c_xhr = $.ajax({ url: url, type: 'POST', data:{ 'ci':ci, 'name':name }, success: function(resp) {
				aj_ip = false;
				resp = $.trim(resp);
...
function biGChat() {
	$('#glist > .group > .cmsr').click(function() {
		...
		if (!aj_ip) {
			aj_ip = true;
			c_xhr = $.ajax({ url: url, type: 'POST', data:{ 'gi':gi, 'gid':gid, 'name':name }, success: function(resp) {
				resp = $.trim(resp);
				if(resp.indexOf('success:') != -1 && resp.indexOf('success:') === 0) {
					setChatUi(gi, gid);
					var q = $.trim(resp.replace('success:',''));
...
function initchat() {
	...
	if(unm == '') { unm = $('#uname').html(); }
	if(eid == '') { eid = $('#eml').html(); }
	// if(phon == '') { phon = $('#phn').html(); }
	...
	if(unm != '' && eid != '' && pass != '') {
		...
		if(!aj_ip) {
			aj_ip = true;
			c_xhr = $.ajax({ url:url, type:'POST', async:false, data:{ 'unm':unm, 'eid':eid, 'pass':pass, 'cp':cpcode }, success:function(resp) {
				if(resp.indexOf('success:') != -1 && resp.indexOf('success:') === 0) {
		...
		if(name != '') {
			...
			if(typeof ms.mcevs != 'undefined' && ms.mcevs != null && typeof ms.chattype != 'undefined' && ms.chattype != null && ms.chattype != '') { 	// && ms.chattype != 'ws'
	...
	if(typeof dtls != 'undefined' && dtls != null) {
			...
			if($.trim(msgs) != '') {
				if(parseInt(senderids,10) == 0 || isNaN(parseInt(senderids,10)) || parseInt(plfv,10) == 0 || isNaN(parseInt(plfv,10))) {
					...
					if(uc != 0) {
						...
						$(options[0]).find(options[1]+' > '+cb_selc).html(msgs); 	// +"<br/><hr style='border-style:dashed;' />"
					...
					if(uc != 0) {
						...
						$(options[0]).find(options[1]+' > '+cb_selc).append(msgs); 	// "<br/>"+msgs+"<br/><hr style='border-style:dashed;' />"
						$('.msg>b:contains("'+name+'")').parent().not('.user-text').addClass('user-text');
						var urm = 0;
						if($('.ctabs > span['+el_selc+'] > span').length > 0) {
							urm = $('.ctabs > span['+el_selc+'] > span').html().replace('(','').replace(')','');
				...
				if (window.webkitNotifications) {
					...
					if (msgs.length > 100) {
						...
						notify_msg = strip_tags(notify_msg,'');
						var notification = new Notification('Message ('+cid+')', { body: notify_msg });
					}
					try {
		...
		$('.uflnk').mouseover(function() {
			...
			if (link.indexOf('/download.php?fl=') != -1) {
				link = link.replace('/download.php?fl=','/download.php?'+'name='+name+'&fl=');
				$(this).attr('href', link);
			}
		});
		// set time value as per user timezone
